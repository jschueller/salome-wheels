...
then
  ...
  docker run --rm -v `pwd`:/io salome/ubuntu20 /io/test-wheels-debian.sh 9.14.0 cp38
fi
...
then
  ...
  docker run --rm -v `pwd`:/io salome/ubuntu22 /io/test-wheels-debian.sh 9.14.0 cp310
fi

if test "${abi}" = "cp311"
then
  docker build docker/debian12 -t salome/debian12
  docker run --rm -v `pwd`:/io salome/debian12 /io/test-wheels-debian.sh 9.14.0 cp311
fi
